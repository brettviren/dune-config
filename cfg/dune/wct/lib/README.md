...
```jsonnet
// Compose + finalize a WCT sub-graph.  Each inode declares its service deps via
// `.uses` (see job/parts.jsonnet); pg.uses() pulls them in + orders them.
local g = import "dune/wct/lib/graph.jsonnet";
local graph = g.pipeline([ g.source(src), g.filter(mid), g.sink(snk) ]);
g.application(graph, name="my_app")   // -> WCT config array (services first, then nodes, then Pgrapher)
